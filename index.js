...
async function loadPage() {
  switch (currentPage) {
    ...
    case 'edit':
      ...
      */
      const inputForm = generateEditFormInput();
...
async function addBook(book) {
  try {
    ...
    */
      ...
      // const addBooks = await resp.json();
      // return addBooks;

      if(response.status == 200){
        const addBooks = await response.json()
        book.push(addBooks)
      } else {
...
async function editBook(book) {
  try {
    ...
    */
    ...
    const editBooks = await response.json();
    const index = book.findIndex((item) => item.id == editBooks.id);
    if(index !== -1){
  ...
  try {
    ...
    */
    ...
    if (response.status == 200){
      ...
      console.log("Buku gagal dihapus");
    }
  } catch (error) {
    console.log(error);
    console.log('Terjadi kesalahan saat menghapus buku');
